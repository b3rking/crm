    ...
    function recupererFicheRecuperation($idfiche)
    {
        ...
        $query = $con->prepare("
            SELECT 
                ...
                co.amount
            FROM client cl
            JOIN fiches f ON f.type_fiche = 'recuperation'
            JOIN contenufiches c ON f.ID_fiches = c.ID_fiches
            JOIN tickets t ON f.ID_fiches = t.fiche_id
            JOIN user u ON u.ID_user = f.ID_user
            LEFT JOIN contract co ON co.ID_client = cl.ID_client
            WHERE t.customer_id = cl.ID_client
            AND f.ID_fiches = ?
        ");
        // $query = $con->prepare("
        //     SELECT 
