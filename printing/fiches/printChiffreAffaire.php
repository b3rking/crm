...
class myPDF extends FPDF
{
	...
	function headerTable()
	{
		...
		$this->Ln(15);
		$this->Cell(50,5,"",0,0);
		$this->Cell(100,5,"RAPPORT DE CHIFFRE D'AFFAIRE",0,1);
		$this->Ln(15);
	...
	function viewTable($contract)
	{
		...
		for ($i=1; $i < 13 ; $i++) 
		{ 
			foreach ($contract->getmontant_payer_dans_un_mois($i,2022) as $value) 
			{
				if ($value->devise == 'USD') 
                {
                    $montant_USD = $value->montant;
                    //$monnaie_USD = $value->monnaie;
                    $total_usd += $montant_USD;
                }
                else
                {
                    $montant_monnaie_locale = $value->montant;
                    //$monnaie_locale = $value->monnaie;
                    $total_bif += $montant_monnaie_locale;
                }
			}
			$this->Row([$tb_mois[$i],number_format($montant_monnaie_locale),number_format($montant_USD)]);
		}
		$this->SetFont('Arial','B',12);
		$this->Cell(50,8,'Total',1,0,'C');
	    $this->Cell(50,8,number_format($total_bif).'_BIF',1,0,'C');
		$this->Cell(50,8,number_format($total_usd).'_USD',1,1,'C');

		$this->SetFont('Arial','B',12);
		$this->Ln(15);
		$this->Cell(50,5,iconv('UTF-8', 'windows-1252' , 'Anneé : 2023'),0,0);
		$this->Ln(5);
		$this->Cell(50,8,'MOIS',1,0,'C');
	    $this->Cell(50,8,'MONTANT EN BIF',1,0,'C');
		$this->Cell(50,8,'MONTANT EN USD',1,1,'C');
		...
		for ($i=1; $i < 13 ; $i++) 
		{ 
			foreach ($contract->getmontant_payer_dans_un_mois($i,2023) as $value) 
			{
				if ($value->devise == 'USD') 
                {
                    $montant_USD = $value->montant;
