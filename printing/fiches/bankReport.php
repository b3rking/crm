...
class myPDF extends FPDF
{
	...
	function viewTable($comptabilite,$date1,$date2,$mois,$annee,$idbanque,$condition,$dateAvantFiltre,$periode)
	{
		...
		foreach ($comptabilite->getBanque($idbanque) as $value) 
		{
			...
			foreach ($comptabilite->filtreBankJournal($condition) as $value1) 
	    	{
	    		...
	    		$this->Row([$value1->date_operation,$value1->libelle,number_format($debit),number_format($credit),number_format($soldeInitial)]);
	    	}
	    	$this->SetFont('Arial','',10);
