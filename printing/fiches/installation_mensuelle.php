...
//date('Y-m-d'));
	...
	function header()
	{
		...
		$this->Cell(190,5,'Installation du mois ',0,1,'C');
			$this->Ln(8);
	...
	{	$this->SetFont('Arial','B',11);
		...
		$this->Cell(35,5,'Client',1,0);
		$this->Cell(30,5,'Telephone',1,0);
		$this->Cell(50,5,'Adresse',1,0);
		$this->Cell(35,5,'Mail',1,0);
		$this->Cell(35,5,'Date installation',1,1);
		$this->SetFont('Arial','',9);
		
		$i = 0;
		foreach ($client->installation_du_mois(date('m'),date('Y')) as $value) 
		{
			$i++;
		        $this->Row([$i,$value->Nom_client,$value->telephone,$value->adresse,$value->mail,$value->dateInstallation]);
    	}
