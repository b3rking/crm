...
class myPDF extends FPDF
{
	...
	function header()
	{
		...
		$this->Cell(160,5,'Client en coupure ',0,1,'C');
			$this->Ln(8);
	...
	{	
		...
		$this->Cell(120,5,'Nom client',1,0,'C',1);
		$this->Cell(30,5,'Solde',1,1,'C',1);
    	...
    	foreach ($client->getClientEnCoupure() as $value) 
    	{
    		...
    		//if ($tbMonnaie[1] == 'USD')
                	$total_dette += $value->solde;
            //else $total_dette_monnaie_locale += $solde;
    	}
    	//$this->SetFont('Arial','B',9);
    	//$this->Cell(150,10,'Total '.$tbMonnaie[0],'TLB',0);
    	//$this->Cell(30,10,round($total_dette_USD).' '.$tbMonnaie[0],'TRB',1);

    	$this->SetFont('Arial','B',9);
    	$this->Cell(150,10,'Total '.$tbMonnaie[0],'TLB',0);
    	$this->Cell(30,10,number_format(round($total_dette)).' '.$tbMonnaie[0],'TRB',0);
	}
}
...
$pdf->SetWidths(array(10,20,120,30));
