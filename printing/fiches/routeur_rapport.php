...
class myPDF extends FPDF
{
	...
	function viewTable($equipements,$marque,$nbEq,$nbOut,$dateAvantFiltre,$periode,$client,$equipement,$vehicule)
	{
		...
		//$this->Cell(105,8,'Generer le '.date("F j, Y, g:i a"), 0,1,'C');
		//$this->Ln(5);
		$this->Cell(180,8,"RAPPORT D'ENTREE / SORTIE DE ROUTEUR",1,1,'C');
		$this->SetFont('Arial','',10);
		...
		$quantiteInitial = $nbEq - $nbOut;
		$quantiteProgressif = 0;

		$this->SetFont('Arial','B',10);
		$this->Cell(180,8,$marque,1,1,'C');
		$this->SetFont('Arial','',10);
		...
		if ($quantiteInitial > 0) 
		{
			...
			$quantiteProgressif += $quantiteInitial;
    	...
    	{
    		$equipement_data = preg_split("#[/]+#",$value->equipements);
    		if ($equipement_data[1] == 'entree') 
    		{
    			$in_storeProgressif++;
    			$quantiteProgressif++;
                $description = $equipement_data[2] != '' ? 'ENTREE : '.$equipement_data[2]: 'ENTREE';
    			$this->Row([$equipement_data[0],iconv('UTF-8', 'windows-1252', $description),1,0,$quantiteProgressif]);
    		}
    		else
    		{
    			$out_storeProgressif++;
    			...
    			if ($equipement_data[2] == 'client') 
		        {
		            $destination_detail = $client->afficherUnClentSansContract($equipement_data[3])->fetch()['Nom_client'];
		        }
		        elseif ($equipement_data[2] == 'relais') 
		        {
		            $destination_detail = $equipement->getSecteurById($equipement_data[3])->fetch()['nom_secteur'];
		        }
		        elseif ($equipement_data[2] == 'base') 
		        {
		            $destination_detail = $equipement->getAccessPointById($equipement_data[3])->fetch()['nom'];
		        }
		        elseif ($equipement_data[2] == 'vehicule') 
		        {
		            $vehicule_res = $vehicule->getVehiculeById($equipement_data[3])->fetch();
		            $destination_detail = $vehicule_res['modele']."-".$vehicule_res['marque'];
		        }
    			$this->Row([$equipement_data[0],iconv('UTF-8', 'windows-1252', $destination_detail),0,1,$quantiteProgressif]);
    		}
    	}
    	$this->SetFont('Arial','',10);
		...
		$this->SetFont('Arial','B',10);
		$this->Cell(35,8,$quantiteProgressif,1,1,'L');
	}
}
...
$pdf->viewTable($equipements,$marque,$nbEq,$nbOut,$dateAvantFiltre,$periode,$client,$equipement,$vehicule);
$pdf->Output();
