...
class myPDF extends FPDF
{
	...
	function viewTable($contract,$year)
	{
		...
		$this->Ln(15);
		$this->Cell(50,5,"",0,0);
		$this->Cell(100,5,"RAPPORT DE PAYEMENT MENSUEL",0,1);
		$this->Line(71,35,142,35);
		$this->Ln(15);

		$this->Cell(50,5,iconv('UTF-8', 'windows-1252' , 'Anneé : '.$year),0,0);
		...
		{
			$montant_USD = 0;
			$montant_monnaie_locale = 0; 
			foreach ($contract->getmontant_payer_dans_un_mois($i,$year) as $value) 
            {
                if ($value->devise == 'USD') 
                {
