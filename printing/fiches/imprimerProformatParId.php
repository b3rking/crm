...
/**
 * Custom PDF class for pro-forma invoices
 */
class myPDF extends FPDF
{
    ...
    function getFactureId() { return $this->facture_id; }
    ...
    function footer()
    {
        ...
        $this->Ln();
        $this->Cell(30, 4, 'Pour toute question concernant cette facture, veuillez contacter ', 0, 1);
    ...
    function viewTable()
    {
        ...
        // Table header - OTT column always shown
        ...
        {
            // Currency conversion
            if ($fixe_rate == 1 && $monnaie != $exchange_currency && $monnaie == 'USD') {
            ...
            if ($value2->billing_cycle == 0) {
                $quantite_str   = $value2->quantite . ' Jours';
                $sousTotalPrixU = $prixU;
            } else {
                $quantite_str   = ($value2->billing_cycle == 1) ? $value2->quantite . ' Mois' : $value2->quantite;
                $sousTotalPrixU = $prixU * $value2->quantite;
            }

            // OTT logic - 100 000 BIF per month for recurring monthly services
            $line_ott = 0;
            if ($value2->billing_cycle == 1 && $value2->ID_service != 1) {
                $line_ott = 100000 * $value2->quantite;
            ...
            $prixTTC = $sousTotalPrixU + $prixTva + $prixTvci + $line_ott;

            // Row with OTT always included
            $ligne1 = [
                iconv('UTF-8', 'windows-1252', $Libelle),
                $quantite_str,
                $prixU . ' ' . $monnaie,
                number_format($sousTotalPrixU) . ' ' . $monnaie,
                number_format($prixTva + $prixTvci) . ' ' . $monnaie,
                number_format($sousTotalPrixU + $prixTvci) . ' ' . $monnaie,
                number_format($line_ott) . ' BIF',
                number_format($prixTTC) . ' ' . $monnaie
            ];

            $this->Row($ligne1);

            // Discount accumulation (monthly services only)
        ...
        $this->Cell(20, 5, number_format($totalOtt) . ' BIF', 1, 0, 'L');
        $this->Cell(20, 5, number_format($totalTTC) . ' ' . $monnaie, 1, 1, 'L');

        $this->Ln();

        if ($enable_discounts == 1 && $rediction > 0) {
...
$pdf->AddPage();
$pdf->headerTable();
$pdf->SetWidths([50, 15, 15, 20, 20, 20, 20, 20]);  // 8 columns now always
$pdf->viewTable();
...
//         echo "<pre>";
//         echo "Service: " . $value2->description . "\n";
//         echo "quantite: " . $value2->quantite . "\n";
//         echo "monnaie: $monnaie\n";
//         echo "current ott: $ott\n";
//         echo "line_ott would be: " . ($quantite * 100000) . "\n";
//         echo "totalOtt so far: $totalOtt\n";
//         echo "</pre>";
