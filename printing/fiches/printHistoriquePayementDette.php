...
class myPDF extends FPDF
{
	...
	function header()
	{
		...
		$this->Cell(70,5,'HISTORIQUE DE PAYEMENT DE DETTE',0,0,'C');
	...
	function headerTable($comptabilite)
	{
		...
		$this->Cell(40,5,'LIBELLE',1,0);
		$this->Cell(30,5,'MONTANT',1,0);
		$this->Cell(35,5,'PROVENANCE',1,0);
		$this->Cell(35,5,'EFFECTUER PAR',1,1);
	}
	function viewTable($comptabilite,$condition)
	{
		...
		$i =0;
		foreach ($comptabilite->filtreHistoriquePayementDette($condition) as $value) 
		{
			$i++;
			$total +=$value->montant_paie;
			//$monnaie = $value->monnaie;
			$this->Row([$i,$value->date_histo,$value->nom,$value->montant_paie,$value->provenancePayement,$value->userName]);
		}
		$this->SetFont('Arial','B',10);
		$this->Cell(172,5,'Total :  '.$total,1,0);
	}
}
...
//$pdf->init($mois,$annee);
$pdf->AddPage();
$pdf->headerTable($comptabilite);
$pdf->SetWidths(array(10,22,40,30,35,35));
