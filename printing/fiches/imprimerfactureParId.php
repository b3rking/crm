...
class myPDF extends FPDF
{
	...
	function viewTable($contract,$facture_id,$columns)
	{
		$client = $this->getClient();
		...
		foreach ($contract->recupererFactureToPrint($facture_id) as $value2) 
		{
           ...
           // $prixTva = round($prixTva);
			...
			else
			{
                ...
                {
                    if(strtolower($client['exchange_currency']) == 'bif')
                        $ligne1 = [$Libelle,$quantite,number_format($prixU).' '.$monnaie,number_format($sousTotalPrixU).' '.$monnaie,number_format($prixTvaShow).' '.$monnaie,number_format($prixTTC).' '.$monnaie,$ott.' BIF',number_format($prixTTC+$ott).' '.$monnaie];
...
//		$this->Cell(25,5,number_format($totalTTC).' '.$monnaie,1,1,'L');
        ...
        {
            if(strtolower($client['exchange_currency']) == 'bif')
              $row = ['Total','','',number_format($totalprixU).' '.$monnaie,number_format($totalPrixTvaShow).' '.$monnaie,number_format($totalTTC).' '.$monnaie,$totalOtt.' BIF',number_format($totalTTC+$totalOtt).' '.$monnaie];
        }
        $this->Row($row);
