...
class myPDF extends FPDF
{
	...
	function header()
	{
		...
		$this->Cell(70,5,'Rapport des sorties',0,0,'C');
	...
	function viewTable($comptabilite,$condition)
	{
		...
		$i =0;
		foreach ($comptabilite->filtreSortieBanque($condition) as $value) 
		{
			$i++;
			$total +=$value->montantsorti;
			$monnaie = $value->monnaie;
			$this->Row([$i,$value->date_sortie,$value->montantsorti.' '.$value->monnaie,$value->motif]);
		}
