 <?php
    ...
    class myPDF extends FPDF
    {
        ...
        function header()
        {
            ...
            $this->Ln(15);
            $this->Cell(160, 5, 'Client avec dette ', 0, 1, 'C', 1);
            $this->Ln(10);
        }
        function footer()
        {
        ...
        function viewTable($client, $contract, $comptabilite)
        {
            ...
            $this->Cell(18, 5, 'ID_Client', 1, 0, 'C', 1);
            $this->Cell(70, 5, 'Client', 1, 0, 'C', 1);
            $this->Cell(30, 5, 'Bande passante', 1, 0, 'C', 1);
            ...
            foreach ($client->getClientDelinquants() as $value) {
                $i++;
                $this->Row([$i, $value->billing_number, $value->Nom_client, $value->bandepassante, number_format(round($value->solde)) . ' ' . $tbMonnaie[0]]);
    ...
    $pdf->SetRightMargin(25.2);
    $pdf->AliasNbPages();
    //$pdf->init($facture_id);
    $pdf->AddPage();
    $pdf->headerTable();

    $pdf->SetWidths(array(10, 18, 70, 30, 40));
