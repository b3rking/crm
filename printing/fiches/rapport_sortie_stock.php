...
class myPDF extends FPDF
{
	...
	function header()
	{
		...
		$this->Cell(70,5,'Rapport de sortie de stock',0,0,'C',1);

	}
	function footer()
	{	
	}
	function headerTable($equipement,$condition,$idaccessoire)
	{
		...
		$this->Cell(21,5,'DATE',1,0,'C',1);
		$this->Cell(30,5,'ACCESSOIRE',1,0,'C',1);
		//$this->Cell(30,5,'MOTIF',1,0,'C',1);
		$this->Cell(30,5,'MOTIF',1,0,'C',1);
		$this->Cell(60,5,'CLIENT',1,0,'C',1);
		$this->Cell(32,5,'DEMANDE PAR',1,1,'C',1);
	}
	function viewTable($equipement,$condition)
	{
		//$total = 0;
		//$monnaie ;
		$this->SetFont('Arial','',10);
		$i =0;
		foreach ($equipement->filtreSortie_stock($condition) as $value) 
		{
			$i++;
			//$total +=$value->montant_sortie;
			//$monnaie = $value->devise;

			$this->Row([$i,$value->date_sortie,$value->categorie,$value->motif/*,$value->serviteur*/,$value->Nom_client,$value->nom_user]);
		}
...
//$pdf->init($mois,$annee);
$pdf->AddPage();
$pdf->headerTable($equipement,$condition,$idaccessoire);
$pdf->SetWidths(array(8,21,30,30,60,32));
$pdf->viewTable($equipement,$condition);
$pdf->Output();
?>
