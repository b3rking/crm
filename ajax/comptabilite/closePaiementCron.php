...
try
{
    ...
    foreach ($comptabilite->getPayementCashNotCloseForAllUsers() as $value) 
    {
    ...
    foreach ($comptabilite->getPayementNoCashNotCloseForAllUsers() as $value2) 
    {
        if ($comptabilite->augmanterMontantBanque($value2->ID_banque,$value2->montant) > 0) 
        {
            if ($comptabilite->versement($value2->ID_banque,$value2->reference,$value2->datepaiement,$iduser=45,$value2->montant))  
            {
    ...
    if ($comptabilite->closePayementsByCron(49) > 0) 
    {
        # code...
    }
    $con->commit();
