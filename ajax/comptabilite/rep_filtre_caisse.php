<?php
$i=0;
foreach ($comptabilite->filtreCaisses($condition) as $value) 
{
$i++;
?>
<tr>
  <td><?php echo $value->nomCaisse?></td>
  <td><?php echo $value->montantCaisse?></td>
          ...
          <div class="modal-body">
            <form class="form-horizontal p-t-20">
              ...
              <div class="modal-footer">
                <button type="button" style="background-color: #7c4a2f" class="btn text-white"data-dismiss="modal" onclick="updateCaisse($('#idcaisse<?=$i?>').val(),$('#nomcaisse<?=$i?>').val(),$('#devise<?=$i?>').val(),$('#lignecredit<?=$i?>').val(),$('#statut<?=$i?>').val(),$('#responsable<?=$i?>').val(),$('#datecreationcaisse<?=$i?>').val(),$('#dimmension<?=$i?>').val(),$('#description<?=$i?>').val(),$('#idusers').val())"> <i class="fa fa-check"></i>modifier caisse</button>
