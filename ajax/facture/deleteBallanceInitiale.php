...
if ($contract->deleteBallanceInitiale($_GET['id']) > 0) 
{
	$taux = 1765;
    //$taux = 2000;
	$facture_bif = 0;
...
//	}
	...
	$solde = $facture_bif - $paiement_bif;
    
	$client->updateSoldeClient($_GET['idclient'],$solde);
	if ($historique->setHistoriqueAction($_GET['id'],'balanceInitiale',$_GET['iduser'],date('Y-m-d'),'supprimer')) 
	{
	}
}