...
foreach ($contract->recupererFactures() as $value) 
{
    $i++;
?>
    <tr>
                        ...
                        <div class="modal-body">
                            <form class="form-horizontal p-t-20" name="formaddClient">
                                <div class="row">
                                    ...
                                    <div class="col-lg-5 col-md-3" hidden="">
                                        <div class="row">
                                            ...
                                            <div class="col-sm-9">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        ...
                                                        <input type="text" id="afficheTaux<?=$i?>" value="0">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
?>
    ...
    <div class="row">
        ...
        <div class="col-lg-4 col-md-4">
            ...
            <div class="form-group col-sm-9">
                <textarea class="form-control" id="description<?=$y?>">
                    <?=trim($value2->description)?>
                </textarea>
            </div>
        </div>
        <div class="col-lg-4 col-md-4">
        ...
        <label class="control-label">Jour de plus</label>
        <div class="form-group col-sm-9 col-lg-2">
            <input type="number" id="jourDePlus<?=$y?>" value="<?=$value2->JourDePlus?>" class="form-control">
        </div>
        <label class="control-label">Montant de jours de plus</label>
        <div class="form-group col-sm-9 col-lg-2">
            <input type="number" id="montantjourDePlus<?=$y?>" value="<?=$value2->montantJourDePlus?>" class="form-control">
        </div>
    </div>
    <?php
    if ($value2->billing_cycle == 0) 
    ...
    ?>
        <div class="row">
            <div class="col-lg-4 col-md-4">
                ...
                <div class="form-group col-sm-9">
                    <input type="date" class="form-control" name="startDate" id="startDate<?=$y?>" value="<?=$value2->startDate?>">
                </div>
            </div>
            <div class="col-lg-4 col-md-4">
                <label class="control-label">Date fin</label>
                <div class="form-group col-sm-9">
                    <input type="date" class="form-control" name="endDate" id="endDate<?=$y?>" value="<?=$value2->endDate?>">
                </div>
            </div>
        </div>
    <?php
...
<input type="number" id="id_dernierService<?=$i?>" value="<?=$y?>" hidden>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-2 col-md-2" hidden="">
                                        <div class="row">
                                            ...
                                            <div class="form-group col-sm-9">
                                                <input type="number" class="form-control" id="tva<?=$i?>" value="<?=$value->tva?>" checked>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button class="btn btn-success waves-effect text-left" onclick="updateFacture($('#facture_id<?=$i?>').val(),$('#datefacture<?=$i?>').val(),$('#monnaie<?=$i?>').val(),$('#mois<?=$i?>').val(),$('#annee<?=$i?>').val(),$('#tva<?=$i?>').val(),$('#billing_number<?=$i?>').val(),$('#i<?=$i?>').val(),$('#id_dernierService<?=$i?>').val(),$('#afficheTaux<?=$i?>').val(),$('#taux<?=$i?>').val())" data-dismiss="modal"><i class="fa fa-check"></i>Modifier
                            </button>
            ...
            <div class="modal fade bs-example-modal-sm<?=$i?>" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" style="display: none;">
                <div class="modal-dialog modal-sm">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-danger waves-effect text-left" onclick="deleteFacture($('#id_factureDelete<?=$i?>').val(),$('#del_moisdebut<?=$i?>').val(),$('#del_annedebut<?=$i?>').val())" data-dismiss="modal"><i class="ti-trash"></i>Supprimer
                            </button>
                        </div>
                    </div>
                    <!-- /.modal-content -->
