<?php
	...
	/*$donnee = $equipement->getEquipement_avantsupression($_GET['id'])->fetch();
	if ($donnee) 
	{
		
		if ($equipement->insererHistoriqueStock($donnee['ID_equipement'],$donnee['model'],$donnee['fabriquant'],$donnee['type_equipement'],$donnee['date_stock'],$donnee['ID_user']))
			{
				if ($equipement->deleteEquipement($_GET['id'])) 
