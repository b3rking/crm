...
else
{
    ...
    if ($equipement->ajouterStock(strtoupper($_GET['model']),$_GET['type'],$mac,$nbport,$_GET['date_recuperation'],$description,$_GET['iduser'])) 
    {
        $id = $equipement->getMaxIdEquipement()->fetch()['ID_equipement'];
        if ($nbport > 1) 
        {
            $equipement->ajouterMacAdresse($mac,$id);
        ...
        if ($equipement->setEquipementRecovery($id,$_GET['idclient'],$_GET['date_recuperation'],$_GET['iduser'],$_GET['description'])) 
        {
            # code...
        }
