...
$i = 0;
$nbservice = $contract->getNombreServiceDunContrat($_POST['result'])->fetch()['nbservice'];
foreach ($contract->getServiceToIncludeOnFacture($_POST['result']) as $value)
{
    $i++;
?>
	<div class="row">
    ...
    <div class="row">
        ...
        <div class="form-group col-sm-9 col-lg-2">
            <input type="number" id="jourDePlus<?=$i?>" value="0" class="form-control">
        </div>
        <label class="control-label">Montant de jours de plus</label>
        <div class="form-group col-sm-9 col-lg-2">
            <input type="number" id="montantjourDePlus<?=$i?>" value="0" class="form-control">
        </div>
    </div>
