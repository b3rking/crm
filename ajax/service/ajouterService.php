...
if ($service->ajouterService($_GET['nom'],$_GET['montant'],$_GET['monnaie'],$_GET['description'])) 
{
     $idAction = $service->getMaxIdservice()->fetch()['ID_service'];
     if ($comptabilite->setHistoriqueAction($idAction,'service',$_GET['userName'],date('Y-m-d'),'creer')) 
    {
	$i = 0;
...
<div class="modal-footer">
<button type="button" class="btn waves-effect waves-light btn-rounded btn-danger" onclick="deleteService($('#idservice<?= $i?>').val())" data-dismiss="modal" data-dismiss="modal">Supprimer</button>
</div>
</div>
<!-- /.modal-content -->
</div>
