...
<div class="row">
    ...
    <div class="col-lg-3">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">STATUS DE CLIENTS</h6>
    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Graph mensuel de montant recu</h4>
                <div id="montant_recu_graph"></div>
            </div>
        </div>
    </div>
</div>
<div class="row">
    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Clients payants</h4>
                <div class="table-responsive">
                    <table class="table full-color-table ">
                        ...
                        <tbody>
                            ...
                            <tr onclick="printClientActif($('#url').val())">
                                ...
                                //$sansdette = nbClientSoldeInfOuEgalZero();
                                $sansdette = $client->get_nombre_clientActif_sans_Dette();
                                $encoupure = $client->totalClientEnCoupure()->nb;
                                $actif = $client->totalClientsActif()->nb;
                                $total = $actif + $encoupure;
                                ?>
                                <td><?=$client->totalClientPayants()->nb?></td>
                                <td><?=$actif?></td>
                                <td><?=$enderogation?></td>
                                <td><?=$sansdette?></td>
                                <td><?=$encoupure?></td>
            ...
            <div class="card-body">
                <h4 class="card-title">Facturation du <?php
                echo date("m/Y");?> </h4>
                <div class="table-responsive">
                     <table class="table full-color-table ">
                        <thead style="background-color: #ef7f22" class="text-white">
                            <tr>
                                <th>Facture total</th>
                                <th hidden>En derogation</th>
                                <th>Coupure</th>
                                <th hidden="">Dette total en coupure</th>
                                <th>Total dette </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                ...
                                $montant_facture_USD = 0;
                                $montant_facture_monnaie_locale = 0;
                                foreach ($contract->getfacture_total_dun_mois($mois_courant,$annee_courant) as $value) 
                                {
                                    if ($value->monnaie == 'USD') 
                                    {
                                        $montant_facture_USD = $value->montant;
                                ...
                                echo number_format(round($montant_facture_USD)).'_'.$monnaie_facture_USD.' '.number_format(round($montant_facture_monnaie_locale)).'_'.$monnaie_facture_locale;
                                    ?>
                                </td>
                                <td hidden="hidden">
                                    <?php
                                    /*$soldeDerrogation=0;
                                    foreach ($client->listeclientderoguer() as $value) 
                                    {
                                        $i++;
                                        $soldeDerrogation += $contract->getSommeTotaleFactureDunClient($value->ID_client)->fetch()['montant'] - $contract->getSommeTotalePayementDunClient($value->ID_client)->fetch()['montant'];
                                    }
                                    echo $soldeDerrogation.' USD';*/
                                    ?>
                                </td>
                                <td>
                                    <?php
                                    $montant_coupure_en_cours_usd = 0;
                                    $montant_coupure_en_cours_bif = 0;
                                    foreach ($contract->getMontantCoupureEncours() as $value) 
                                    {
                                        if ($value->monnaie == 'USD') 
                                            $montant_coupure_en_cours_usd = $value->montant;
                                        else $montant_coupure_en_cours_bif = $value->montant;
                                    }
                                    echo number_format($montant_coupure_en_cours_usd).'_USD '.number_format($montant_coupure_en_cours_bif).'_BIF';
                                    ?>
                                </td>
                                <td hidden="">
                                    <?php
                                    $montantDetteEnCoupure = 0;
                                    foreach ($client->getClientEnCoupure() as $value) 
                                    {
                                        $montantDetteEnCoupure += $value->solde;
                                    }
                                    /*$res = $contract->getMontantDetteClientEncoupure()->fetch();*/
                                    echo number_format($montantDetteEnCoupure,2).'_USD';
                                    ?>
                                </td>
                                <td>
                                    <?php
                                    ...
                                    foreach ($contract->getmontant_facture_impayee_dun_mois($mois_courant,$annee_courant) as $value) 
                                    {
                                        if ($value->monnaie == 'USD') 
                                        {
                                    ...
                                    echo round($montant_facture_impayee_USD).'_'.$tbMonnaie[0].' '.round($montant_facture_impayee_monnaie_locale).'_'.$tbMonnaie[1];
                                    ?>
                                </td>
                            </tr>           
                        </tbody>
...
<div class="row">
                ...
                <div class="table-responsive">
                     <table class="table full-color-table ">
                        ...
                        <?php
                            ...
                            ?>
                            <tr style="background-color: #ef7f22;color:white">
                                <td>Total</td>
                                <td>
                                     <?php
    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">SOLDE MENSUEL DE LA TRESORERIE</h4>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    $montant_payer_USD = 0;
                                    $montant_payer_monnaie_local = 0;
                                    $entree_mois_courant_USD = 0;
                                    $entree_mois_courant_monnaie_locale = 0;
                                    foreach ($contract->getmontant_payer_dans_un_mois($mois,$annee) as $value) 
                                    {
                                        if ($value->devise == 'USD')
                                        {
                                            $montant_payer_USD = $value->montant;
                                            $entree_mois_courant_USD +=$value->montant;
                                        }
                                        else
                                        {
                                            $montant_payer_monnaie_local = $value->montant;
                                            $entree_mois_courant_monnaie_locale = $value->montant;
                                        }
                                    }
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $extrat_mois_courant_USD = 0;
                                    $extrat_mois_courant_monnaie_locale =0;
                                    foreach ($comptabilite->total_extrat_dun_mois($mois,$annee) as $value) 
                                    {
                                        if ($value->monnaie == 'USD')
                                        {
                                            $extrat_mois_courant_USD = $value->montant;
                                            $entree_mois_courant_USD +=$value->montant;
                                        }
                                        else
                                        {
                                            $extrat_mois_courant_monnaie_locale = $value->montant;

                                            $entree_mois_courant_monnaie_locale += $value->montant; 
                                        }
                                    }
                                    echo number_format($extrat_mois_courant_USD).' '.$tbMonnaie[0].' '.number_format($extrat_mois_courant_monnaie_locale).' '.$tbMonnaie[1];
                                    ?>
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                echo number_format($depense_mois_courant_USD).'_'.$tbMonnaie[0].' '.number_format($depense_mois_courant_monnaie_locale).'_'.$tbMonnaie[1];
                                ?>
                                </td>
                            </tr>
                            <tr>
    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table full-color-table nowrap">
                        <thead style="background-color: #ef7f22" class="text-white">
                            <tr>
                                <th>Banque</th>
                                <th>Montant</th>
                        ...
                        <tbody>
                            ...
                            foreach ($comptabilite->getCaisseRecettes() as $value) 
                            {
                                if ($value->devise == 'USD')
